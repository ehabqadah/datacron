package de.fhg.iais.spark.cassandra;
...
public class Cassandra implements Serializable {
	...
	private static final long serialVersionUID = -8323496577373087036L;
	...
	public void executeKeyspaceQuery() {
		...
		keyspaceQuery.append(" WITH REPLICATION = { 'class':'" + this.replicationStrategy + "', 'replication_factor':"
	...
	public <T> CassandraJavaPairRDD<String, T> readTableRowsByKey(String tableName, Class<T> tableClass, String... keys){
		return javaFunctions(sparkContext).cassandraTable(this.keyspace, tableName, mapRowTo(tableClass)).keyBy(mapRowTo(String.class), String.class, keys);
	}
